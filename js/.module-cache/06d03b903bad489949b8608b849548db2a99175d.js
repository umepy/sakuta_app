    ...
    function(resolve, reject) {
      ...
      .then((responseText) => {
        ...
        var adescription=$(responseText).filter("meta[property='og:description']").attr('content')
          || $(responseText).text().trim().slice(0, 50) || "Sorry, it failed to load."
        var aimage =$(responseText).filter("meta[property='og:image']").attr('content')

        console.log($(responseText).text().trim().slice(0, 50))
        console.log($(responseText).filter("div[id='main']").text())
